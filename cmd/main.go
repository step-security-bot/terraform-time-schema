...
import (
	...
	"github.com/lonegunmanb/terraform-time-schema/schema"
	"golang.org/x/oauth2"
)

func main() {
	refreshFlag := flag.Bool("refresh", false, "Refresh time schema")
...
func checkGitHubTag(schemaVersion string) (bool, error) {
	...
	for {
		tags, resp, err := client.Repositories.ListTags(context.Background(), "lonegunmanb", "terraform-time-schema", options)
		if err != nil {
			return false, err
		}
